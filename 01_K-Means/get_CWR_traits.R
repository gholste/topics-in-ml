...
tmp <- read.csv("~/Desktop/Spring2019/BIOL385/CWR/Data/01_CWR_species_by_cell.csv")
...
traits_of_interest <- c("leaf area per leaf dry mass", "whole plant height",
...
# species_of_interest <- intersect(s1, s2); length(species_of_interest)
...
colnames(df) <- c("Species", "Leaf Area per Leaf Dry Mass",
                  "Whole Plant Height", "Diameter at Breast Height")
